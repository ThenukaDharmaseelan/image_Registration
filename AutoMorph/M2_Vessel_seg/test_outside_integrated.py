...
#     test_dir = f'{AUTOMORPH_DATA}/images/'
...
def filter_frag(data_path):
    """Process binary segmentations and calculate fractal dimensions"""
    ...
    for i in sorted(image_list):
        ...
        per_image_filter_times.append(time.time() - img_start)
    
    filter_total_time = time.time() - filter_start
    
    return FD_cal, name_list, VD_cal, width_cal, filter_total_time, per_image_filter_times


def segment_fundus(data_path, net_1, net_2, net_3, net_4, net_5, net_6, net_7, net_8, net_9, net_10, loader, device, dataset_name, job_name, mask_or, train_or):
    """Perform segmentation with ensemble of models"""
    ...
    with tqdm(total=n_val, desc='Segmentation', unit='batch', leave=False) as pbar:
        for batch in loader:
            ...
            # INFERENCE - Ensemble of 10 models
            ...
            for i in range(n_image):
                ...
                per_image_times.append(time.time() - img_save_start)
            
            save_times.append(time.time() - save_start)
            per_batch_times.append(time.time() - batch_start)
            
            pbar.update(imgs.shape[0])
    
    return per_batch_times, per_image_times, inference_times, postprocess_times, save_times


def test_net(data_path, batch_size, device, dataset_train, dataset_test, image_size, job_name, threshold, checkpoint_mode, mask_or=True, train_or=False, save_runtime=False):
    ...
    stage_times['model_loading'] = time.time() - model_load_start
    
    # SEGMENTATION
    seg_start = time.time()
    per_batch_times, per_image_times, inference_times, postprocess_times, save_times_seg = segment_fundus(
        data_path, net_1, net_2, net_3, net_4, net_5, net_6, net_7, net_8, net_9, net_10, 
        test_loader, device, dataset_train, job_name, mask_or, train_or
    )
    stage_times['segmentation'] = time.time() - seg_start
    
    # FILTERING AND POST-PROCESSING
    filter_start = time.time()
    FD_list, Name_list, VD_list, width_cal, filter_total_time, per_image_filter_times = filter_frag(data_path)
    stage_times['filtering'] = filter_total_time
    
    # SAVING RESULTS
    save_start = time.time()
    if not os.path.exists(f'{AUTOMORPH_DATA}/Results/M3/'):
        os.makedirs(f'{AUTOMORPH_DATA}/Results/M3/')
    stage_times['saving'] = time.time() - save_start
    ...
    # ========================================
    print("\n" + "="*70)
    print("SEGMENTATION RUNTIME STATISTICS")
    print("="*70)
    
    print(f"\nTotal images processed: {len(Name_list)}")
    print(f"Batch size: {batch_size}")
    print(f"Device: {device}")
    print(f"Number of ensemble models: 10")
    
    print(f"\n{'─'*70}")
    print("STAGE-WISE BREAKDOWN:")
    print(f"{'─'*70}")
    print(f"  Setup & Data Loading:  {stage_times['setup']:8.2f}s ({stage_times['setup']/total_runtime*100:5.1f}%)")
    print(f"  Model Loading (10x):   {stage_times['model_loading']:8.2f}s ({stage_times['model_loading']/total_runtime*100:5.1f}%)")
    print(f"  Segmentation:          {stage_times['segmentation']:8.2f}s ({stage_times['segmentation']/total_runtime*100:5.1f}%)")
    print(f"  Filtering & Analysis:  {stage_times['filtering']:8.2f}s ({stage_times['filtering']/total_runtime*100:5.1f}%)")
    print(f"  Saving Results:        {stage_times['saving']:8.2f}s ({stage_times['saving']/total_runtime*100:5.1f}%)")
    
    if per_batch_times:
        print(f"\n{'─'*70}")
        print("BATCH-LEVEL STATISTICS:")
        print(f"{'─'*70}")
        print(f"  Total batches: {len(per_batch_times)}")
        print(f"  Avg time per batch:    {np.mean(per_batch_times):.4f}s")
        print(f"  Min time per batch:    {np.min(per_batch_times):.4f}s")
        print(f"  Max time per batch:    {np.max(per_batch_times):.4f}s")
        
    if inference_times:
        print(f"\n{'─'*70}")
        print("INFERENCE STATISTICS (Ensemble of 10 models):")
        print(f"{'─'*70}")
        print(f"  Total inference time:  {sum(inference_times):.2f}s")
        print(f"  Avg per batch:         {np.mean(inference_times):.4f}s")
        print(f"  Avg per model/batch:   {np.mean(inference_times)/10:.4f}s")
        
    if postprocess_times:
        print(f"\n{'─'*70}")
        print("POST-PROCESSING (Sigmoid + Uncertainty):")
        print(f"{'─'*70}")
        print(f"  Total time:            {sum(postprocess_times):.2f}s")
        print(f"  Avg per batch:         {np.mean(postprocess_times):.4f}s")
    
    if per_image_filter_times:
        print(f"\n{'─'*70}")
        print("FILTERING & FEATURE EXTRACTION (per image):")
        print(f"{'─'*70}")
        print(f"  Total images:          {len(per_image_filter_times)}")
        print(f"  Avg time per image:    {np.mean(per_image_filter_times):.4f}s")
        print(f"  Min time per image:    {np.min(per_image_filter_times):.4f}s")
        print(f"  Max time per image:    {np.max(per_image_filter_times):.4f}s")
        print(f"  Throughput:            {len(per_image_filter_times)/sum(per_image_filter_times):.2f} images/sec")
    ...
    if save_runtime:
        runtime_stats = {
            ...
            'batch_statistics': {
                'total_batches': len(per_batch_times) if per_batch_times else 0,
                'avg_time_per_batch': float(np.mean(per_batch_times)) if per_batch_times else None,
                'min_time_per_batch': float(np.min(per_batch_times)) if per_batch_times else None,
                'max_time_per_batch': float(np.max(per_batch_times)) if per_batch_times else None,
                'all_batch_times': per_batch_times
            },
            'inference_statistics': {
                'total_inference_time': sum(inference_times) if inference_times else 0,
                'avg_per_batch': float(np.mean(inference_times)) if inference_times else None,
                'avg_per_model_per_batch': float(np.mean(inference_times)/10) if inference_times else None,
                'all_inference_times': inference_times
            },
            'postprocessing_statistics': {
                'total_time': sum(postprocess_times) if postprocess_times else 0,
                'avg_per_batch': float(np.mean(postprocess_times)) if postprocess_times else None,
                'all_postprocess_times': postprocess_times
            },
            'filtering_statistics': {
                'total_images': len(per_image_filter_times) if per_image_filter_times else 0,
                'total_time': sum(per_image_filter_times) if per_image_filter_times else 0,
                'avg_per_image': float(np.mean(per_image_filter_times)) if per_image_filter_times else None,
                'min_per_image': float(np.min(per_image_filter_times)) if per_image_filter_times else None,
                'max_per_image': float(np.max(per_image_filter_times)) if per_image_filter_times else None,
                'throughput_images_per_sec': len(per_image_filter_times)/sum(per_image_filter_times) if per_image_filter_times else None,
                'all_filter_times': per_image_filter_times
            },
            'per_image_save_times': per_image_times
        }
        
        runtime_file = os.path.join(data_path, 'segmentation_runtime_statistics.json')
...
if __name__ == '__main__':
    ...
    test_net(data_path=args.data_path,
             ...
             mask_or=True, 
             train_or=False,
             save_runtime=args.save_runtime)