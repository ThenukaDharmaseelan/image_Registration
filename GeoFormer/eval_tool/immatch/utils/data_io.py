...
def load_gt_points(gt_file, H_pred):
    """
    Load ground-truth points and compute transformed predicted points.
    Assumes FIRE ground truth files contain two columns: x y (GT points).
    """
    # Load ground-truth keypoints
    gt_pts = np.loadtxt(gt_file)
    if gt_pts.ndim == 1:
        gt_pts = gt_pts[None, :]  # single point case

    # Convert to homogeneous coordinates
    ones = np.ones((gt_pts.shape[0], 1))
    gt_homo = np.concatenate([gt_pts, ones], axis=1)

    # Apply predicted homography
    pred_homo = (H_pred @ gt_homo.T).T
    pred_homo = pred_homo / pred_homo[:, [2]]

    pred_pts = pred_homo[:, :2]
